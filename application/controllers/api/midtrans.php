    ...
    public function index()
	{
        ...
        if($res['status_code'] == 201 || $res['status_code'] == 200) {
            if($res['transaction_status'] == 'settlement') {
                ...
                if ($bank === 'UNKNOWN') {
                    if (isset($res['issuer'])) {
                        $bank = strtoupper($res['issuer']);
                    } elseif (isset($res['store'])) {
                        $bank = strtoupper($res['store']);
                    } elseif (isset($res['payment_type'])) {
                ...
                $ins['Description']       = "Added By Midtrans - Method: " . $bank . $gross_amount; // Deskripsi dengan bank
